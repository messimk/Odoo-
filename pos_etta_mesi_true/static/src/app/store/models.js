...
patch(Order.prototype, {
    ...
    // FS Polling related methods
    hasValidFS() {
        return this.is_refund ? this.rf_no !== "" : this.fs_no !== "";
    },

    needsFSNumber() {
        return this.finalized && !this.hasValidFS() && this.amount_total > 0;
    },

    updateFromFSPolling(fsData) {
        console.log(`Updating order ${this.pos_reference} with FS data:`, fsData);

        if (fsData.fs_no && fsData.fs_no !== "") {
            this.set_fs_no(fsData.fs_no);
        }
        if (fsData.rf_no && fsData.rf_no !== "") {
            this.set_rf_no(fsData.rf_no);
        }
        if (fsData.fiscal_mrc && fsData.fiscal_mrc !== "") {
            this.set_fiscal_mrc(fsData.fiscal_mrc);
        }
        if (fsData.ej_checksum && fsData.ej_checksum !== "") {
            this.set_ej_checksum(fsData.ej_checksum);
        }

        // Mark as fiscally printed
        this.fiscal_printed = true;

        // Update local database
        this.save_to_db();

        console.log(`Order ${this.pos_reference} successfully updated with FS number`);
    },
    ...
    async printFiscalReceipt() {
        ...
        if (this.isPrinted) {
            console.log("🚫 Receipt already printed for order:", this.name);
        ...
        if (window.Android != undefined) {
            if (window.Android.isAndroidPOS()) {
                ...
                if (responseObject.success) {
                    ...
                    this.isPrinted = true;
                    console.log("✅ Receipt printed successfully - isPrinted flag set to true");
